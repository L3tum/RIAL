...
class TestExternalFunction(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        super(cls, TestExternalFunction).setUpClass()
        cls.dir_path = os.path.join(os.path.abspath("/".join(f"{__file__}".split('/')[0:-1])), "TestExternalFunction")
    ...
    def test_correct_ir(self):
        ...
        self.assertIn('declare void @"printf"(i8* %"format", ...) ', content)
